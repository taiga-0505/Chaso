...
/// 最大 kMaxAreaLights つのアクティブなエリアライトを管理し、一括して GPU (b5スロット)へ転送します。
class AreaLightManager {
public:
  /// @brief 同時に有効化可能なエリアライトの最大数
  /// @details struct.h の kMaxAreaLights（= HLSL の MAX_AREA_LIGHTS）と同じ値にする
  static constexpr int kMaxActive = static_cast<int>(kMaxAreaLights);
